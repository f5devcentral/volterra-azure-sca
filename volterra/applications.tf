####create origin pool for APM SL####
resource "volterra_origin_pool" "apm" {
  name      = "apm"
...
#####APM HTTPS LB#####
resource "volterra_http_loadbalancer" "apm" {
  name      = "apm"
  namespace = var.namespace

  depends_on = [
    volterra_tf_params_action.action_test, volterra_origin_pool.apm
  ]
  ...
  domains = ["apm.${var.delegated_domain}"]
